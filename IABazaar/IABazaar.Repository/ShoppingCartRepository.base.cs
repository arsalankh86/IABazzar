...
namespace IABazaar.Repository
{
	...
	public abstract partial class ShoppingCartRepositoryBase : Repository 
	{
        ...
        public virtual DataSet GetAllShoppingCartAndProductDetailByCustomerID(int CustomerID)
        {
            ...
            string sql = "select p.ProductID, c.ProductPrice, c.Quantity, c.TaxRate as CartTotal, p.ImageFilenameOverride,  p.Name, c.CustomerID, c.ShoppingCartRecID from ShoppingCart c, Product p where c.ProductID = p.ProductID and c.CustomerID = " + CustomerID + " ";
            DataSet ds = SqlHelper.ExecuteDataset(this.ConnectionString, CommandType.Text, sql, null);
            if (ds == null || ds.Tables[0].Rows.Count == 0) return null;
            return ds;
        }
