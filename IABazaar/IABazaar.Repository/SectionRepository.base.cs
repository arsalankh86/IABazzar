...
namespace IABazaar.Repository
{
	...
	public abstract partial class SectionRepositoryBase : Repository 
	{
        
        public SectionRepositoryBase()
        {   
			...
			this.SearchColumns.Add("SectionGUID",new SearchColumn(){Name="SectionGUID",Title="SectionGUID",SelectClause="SectionGUID",WhereClause="AllRecords.SectionGUID",DataType="System.Guid",IsForeignColumn=false,IsAdvanceSearchColumn = false,IsBasicSearchColumm = false});
			...
			this.SearchColumns.Add("ParentSectionID",new SearchColumn(){Name="ParentSectionID",Title="ParentSectionID",SelectClause="ParentSectionID",WhereClause="AllRecords.ParentSectionID",DataType="System.Int32",IsForeignColumn=false,IsAdvanceSearchColumn = false,IsBasicSearchColumm = false});
			...
			this.SearchColumns.Add("AllowCategoryFiltering",new SearchColumn(){Name="AllowCategoryFiltering",Title="AllowCategoryFiltering",SelectClause="AllowCategoryFiltering",WhereClause="AllRecords.AllowCategoryFiltering",DataType="System.Byte",IsForeignColumn=false,IsAdvanceSearchColumn = false,IsBasicSearchColumm = false});
		...
		public virtual List<SearchColumn> GetSectionSearchColumns()
        {
        ...
        public virtual Dictionary<string, string> GetSectionBasicSearchColumns()
        {
        ...
        public virtual List<SearchColumn> GetSectionAdvanceSearchColumns()
        {
            List<SearchColumn> searchColumns = new List<SearchColumn>();
            foreach (KeyValuePair<string, SearchColumn> keyValuePair in this.SearchColumns)
            {
        ...
        public virtual string GetSectionSelectClause()
        {
            List<SearchColumn> searchColumns = new List<SearchColumn>();
            ...
            foreach (KeyValuePair<string, SearchColumn> keyValuePair in this.SearchColumns)
            {
                if (!keyValuePair.Value.IgnoreForDefaultSelect)
                {
                	...
                	else
                	{
                    	if (string.IsNullOrEmpty(selectQuery))
                    	{
                        	selectQuery =  "Section."+keyValuePair.Key;
                    	}
                    	else
                    	{
                        	selectQuery += ",Section."+keyValuePair.Key;
		...
		public virtual Section GetSection(System.Int32 SectionId)
		{

			string sql=GetSectionSelectClause();
			sql+="from Section where SectionID=@SectionID ";
			SqlParameter parameter=new SqlParameter("@SectionID",SectionId);
			DataSet ds=SqlHelper.ExecuteDataset(this.ConnectionString,CommandType.Text,sql,new SqlParameter[] { parameter });
			if (ds == null || ds.Tables.Count != 1 || ds.Tables[0].Rows.Count != 1) return null;
			return SectionFromDataRow(ds.Tables[0].Rows[0]);
		}

		public virtual List<Section> GetAllSection()
		{
			...
			return CollectionFromDataSet<Section>(ds, SectionFromDataRow);
		}

		public virtual List<Section> GetPagedSection(string orderByClause, int pageSize, int startIndex,out int count, List<SearchColumn> searchColumns)
		{
            ...
            count=GetSectionCount(whereClause, searchColumns);
			if(count>0)
			{
            ...
            string tempsql = "INSERT INTO #PageIndex ([SectionID])";
            tempsql += " SELECT ";
            if (pageSize > 0) tempsql += "TOP " + PageUpperBound.ToString();
            tempsql += " [SectionID] ";
            tempsql += " FROM [Section] AllRecords";
            if (!string.IsNullOrEmpty(whereClause) && whereClause.Length > 0) tempsql += " WHERE " + whereClause;
            if (orderByClause.Length > 0) 
			{
				tempsql += " ORDER BY " + orderByClause;
				if( !orderByClause.Contains("SectionID"))
					tempsql += " , (AllRecords.[SectionID])"; 
			}
			else 
			{
				tempsql  += " ORDER BY (AllRecords.[SectionID])"; 
			}           
            
            // Return paged results
            string pagedResultsSql =
                GetSectionSelectClause()+@" FROM [Section], #PageIndex PageIndex WHERE ";
            pagedResultsSql += " PageIndex.IndexId > " + PageLowerBound.ToString(); 
            pagedResultsSql += @" AND [Section].[SectionID] = PageIndex.[SectionID] 
		...
		private int GetSectionCount(string whereClause, List<SearchColumn> searchColumns)
		{

			string sql=string.Empty;
			if(string.IsNullOrEmpty(whereClause))
				sql = "SELECT Count(*) FROM Section as AllRecords ";
			else
				sql = "SELECT Count(*) FROM Section as AllRecords where  " +whereClause;
			var rowCount = SqlHelper.ExecuteScalar(this.ConnectionString, CommandType.Text, sql, GetSQLParamtersBySearchColumns(searchColumns));
			return rowCount == DBNull.Value ? 0 :(int)rowCount;
		}

		[MOLog(AuditOperations.Create,typeof(Section))]
		public virtual Section InsertSection(Section entity)
		{
			...
			{
				string sql=@"Insert into Section ( [SectionGUID]
				,[Name]
				,[Summary]
				,[Description]
				,[SEKeywords]
				...
				SqlParameter[] parameterArray=new SqlParameter[]{
					...
					, new SqlParameter("@SectionGUID",entity.SectionGuid)
					...
					, new SqlParameter("@ParentSectionID",entity.ParentSectionId)
					...
					, new SqlParameter("@AllowCategoryFiltering",entity.AllowCategoryFiltering)
				...
				return GetSection(Convert.ToInt32(identity));
			}
			return entity;
		}

		[MOLog(AuditOperations.Update,typeof(Section))]
		public virtual Section UpdateSection(Section entity)
		{

			if (entity.IsTransient()) return entity;
			Section other = GetSection(entity.SectionId);
			if (entity.Equals(other)) return entity;
			string sql=@"Update Section set  [SectionGUID]=@SectionGUID
							, [Name]=@Name
							, [Summary]=@Summary
							...
							, [AllowCategoryFiltering]=@AllowCategoryFiltering
			...
			return GetSection(entity.SectionId);
		}

		public virtual bool DeleteSection(System.Int32 SectionId)
		{
		...
		[MOLog(AuditOperations.Delete,typeof(Section))]
		public virtual Section DeleteSection(Section entity)
		{
			this.DeleteSection(entity.SectionId);
			return entity;
		}


		public virtual Section SectionFromDataRow(DataRow dr)
		{
			if(dr==null) return null;
			Section entity=new Section();
			entity.SectionId = (System.Int32)dr["SectionID"];
			entity.SectionGuid = (System.Guid)dr["SectionGUID"];
			...
			entity.ParentSectionId = (System.Int32)dr["ParentSectionID"];
			...
			entity.AllowCategoryFiltering = (System.Byte)dr["AllowCategoryFiltering"];
