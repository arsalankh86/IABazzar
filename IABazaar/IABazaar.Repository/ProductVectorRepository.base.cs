...
namespace IABazaar.Repository
{
	...
	public abstract partial class ProductVectorRepositoryBase : Repository 
	{
        
        public ProductVectorRepositoryBase()
        {   
		...
		public virtual List<SearchColumn> GetProductVectorSearchColumns()
        {
        ...
        public virtual Dictionary<string, string> GetProductVectorBasicSearchColumns()
        {
        ...
        public virtual List<SearchColumn> GetProductVectorAdvanceSearchColumns()
        {
            List<SearchColumn> searchColumns = new List<SearchColumn>();
            foreach (KeyValuePair<string, SearchColumn> keyValuePair in this.SearchColumns)
            {
        ...
        public virtual string GetProductVectorSelectClause()
        {
            List<SearchColumn> searchColumns = new List<SearchColumn>();
            ...
            foreach (KeyValuePair<string, SearchColumn> keyValuePair in this.SearchColumns)
            {
                if (!keyValuePair.Value.IgnoreForDefaultSelect)
                {
                	...
                	else
                	{
                    	if (string.IsNullOrEmpty(selectQuery))
                    	{
                        	selectQuery =  "ProductVector."+keyValuePair.Key;
                    	}
                    	else
                    	{
                        	selectQuery += ",ProductVector."+keyValuePair.Key;
		...
		public virtual ProductVector GetProductVector(System.Int32 ProductId)
		{

			string sql=GetProductVectorSelectClause();
			sql+="from ProductVector where ProductID=@ProductID ";
			SqlParameter parameter=new SqlParameter("@ProductID",ProductId);
			DataSet ds=SqlHelper.ExecuteDataset(this.ConnectionString,CommandType.Text,sql,new SqlParameter[] { parameter });
			if (ds == null || ds.Tables.Count != 1 || ds.Tables[0].Rows.Count != 1) return null;
			return ProductVectorFromDataRow(ds.Tables[0].Rows[0]);
		}

		public virtual List<ProductVector> GetAllProductVector()
		{

			string sql=GetProductVectorSelectClause();
			sql+="from ProductVector";
			DataSet ds=SqlHelper.ExecuteDataset(this.ConnectionString,CommandType.Text,sql,null);
			if (ds == null || ds.Tables.Count != 1 || ds.Tables[0].Rows.Count == 0) return null;
			return CollectionFromDataSet<ProductVector>(ds, ProductVectorFromDataRow);
		}

		public virtual List<ProductVector> GetPagedProductVector(string orderByClause, int pageSize, int startIndex,out int count, List<SearchColumn> searchColumns)
		{
            ...
            count=GetProductVectorCount(whereClause, searchColumns);
			if(count>0)
			{
            ...
            tempsql += " [ProductID] ";
            tempsql += " FROM [ProductVector] AllRecords";
            ...
            string pagedResultsSql =
                GetProductVectorSelectClause()+@" FROM [ProductVector], #PageIndex PageIndex WHERE ";
            pagedResultsSql += " PageIndex.IndexId > " + PageLowerBound.ToString(); 
            pagedResultsSql += @" AND [ProductVector].[ProductID] = PageIndex.[ProductID] 
		...
		private int GetProductVectorCount(string whereClause, List<SearchColumn> searchColumns)
		{

			string sql=string.Empty;
			if(string.IsNullOrEmpty(whereClause))
				sql = "SELECT Count(*) FROM ProductVector as AllRecords ";
			else
				sql = "SELECT Count(*) FROM ProductVector as AllRecords where  " +whereClause;
			var rowCount = SqlHelper.ExecuteScalar(this.ConnectionString, CommandType.Text, sql, GetSQLParamtersBySearchColumns(searchColumns));
			return rowCount == DBNull.Value ? 0 :(int)rowCount;
		}

		[MOLog(AuditOperations.Create,typeof(ProductVector))]
		...
		[MOLog(AuditOperations.Update,typeof(ProductVector))]
		...
		public virtual bool DeleteProductVector(System.Int32 ProductId)
		{

			string sql="delete from ProductVector where ProductID=@ProductID";
			SqlParameter parameter=new SqlParameter("@ProductID",ProductId);
			var identity=SqlHelper.ExecuteScalar(this.ConnectionString,CommandType.Text,sql,new SqlParameter[] { parameter });
			return (Convert.ToInt32(identity))==1? true: false;
		}

		[MOLog(AuditOperations.Delete,typeof(ProductVector))]
